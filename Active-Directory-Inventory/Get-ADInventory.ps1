<#
        ...
        * LinkedIn:	http://ch.linkedin.com/in/catanesi
        ...
        v1.1,  24/01/2014 - Corrected issue with custom PSObject initialization
#>
...
$adForestInfo | Add-Member -MemberType NoteProperty -Name 'Forest Name' -Value $adForest
...
# Create section for Active Directory Sites
$htmlBody += '<h3>Active Directory Sites Details</h3>'
$adSiteInfo | Add-Member -MemberType NoteProperty -Name 'Sites in Forest' -Value $($adForest.Sites)
$htmlBody += $adSiteInfo | ConvertTo-Html -Fragment -Property 'Sites in Forest'
...
foreach ($domain in $adDomains)
{
    ...
    $htmlBody += $adDomainInfo | ConvertTo-Html -Fragment
    $htmlBody += $htmlBr
